            ...
            for i in range(0, n):
                for j in range(i, n):
                    if i == j:
                        ...
                        criteriaradio = st.radio("Select higher priority criterion ", (criterias[i], criterias[j],),
                                                 horizontal=True)
                        if criteriaradio == criterias[i]:
                            A[i][j] = st.slider(
                                label="how much higher " + criterias[i] + " is in comparision with " + criterias[
                                    j] + " ?", min_value=1, max_value=9, value=1)
                            A[j][i] = float(1 / A[i][j])
                        else:
                            A[j][i] = st.slider(
                                label="how much higher " + criterias[j] + " is in comparision with " + criterias[
                                    i] + " ?", min_value=1, max_value=9, value=1)
                            A[i][j] = float(1 / A[j][i])
                    ...
                    for j in range(i, m):
                        if i == j:
                            ...
                            alternativeradio = st.radio(
                                "Select higher priority alternative for criteria " + criterias[k],
                                (alternatives[i], alternatives[j],), horizontal=True)
                            if alternativeradio == alternatives[i]:
                                B[k][i][j] = st.slider(
                                    "Considering Criterion " + criterias[k] + ", how much higher " + alternatives[
                                        i] + " is in comparision with " + alternatives[j] + " ?", 1, 9, 1)
                                B[k][j][i] = float(1 / B[k][i][j])
                            else:
                                B[k][j][i] = st.slider(
                                    "Considering Criterion " + criterias[k] + ", how much higher " + alternatives[
                                        j] + " is in comparision with " + alternatives[i] + " ?", 1, 9, 1)
